...
print(f'Chạy kịch bản {scence_name}: {ret}')
time.sleep(2)
...
print(f'Lấy thuộc tính thiết bị {name}:\nĐộ sáng: {brightness}%\n'
      f'Nhiệt độ màu: {color_temperature}K')
time.sleep(2)
...
print(f'Thiết lập thuộc tính thiết bị {name}:\n'
      f'Độ sáng: {"Thành công" if ret[0]["code"] == 0 else "Thất bại"}\n'
      f'Nhiệt độ màu: {"Thành công" if ret[1]["code"] == 0 else "Thất bại"}')

# ---------------------- run action ----------------------------
did = devices[0]['did']
ret = api.run_action({"did": did, "siid": 2, "aiid": 1})
print(f'Chuyển đổi công tắc: {"Thành công" if ret["code"] == 0 else "Thất bại"}')
